...
parser = argparse.ArgumentParser(description='E2FGVI_O')
...
def main_worker():
    ...
    for video_no in range(video_num):
        ...
        for f in range(0, video_length):
            ...
            with torch.no_grad():
                selected_imgs = imgs[:1, neighbor_ids+[f]+ref_ids, :, :, :]
                selected_masks = masks[:1, neighbor_ids+[f]+ref_ids, :, :, :]
                masked_imgs = selected_imgs*(1-selected_masks)

                pred_img, _ = model(masked_imgs, len(neighbor_ids)+1)

                pred_img = (pred_img + 1) / 2
                pred_img = pred_img.cpu().permute(0, 2, 3, 1).numpy()*255
                img = np.array(pred_img[len(neighbor_ids)]).astype(np.uint8)*binary_masks[f] + frames[f] * (1-binary_masks[f])
                comp_frames[f] = img
        ...
        if args.save_results:
            folder = os.path.join(args.save_folder,'E2FGVI_O',video_name)
