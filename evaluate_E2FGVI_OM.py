...
def main_worker():
    ...
    for video_no in range(video_num):
        ...
        #Here we store calculated values for the two windows (coarse and fine) in the focal transformers
        #And also calculated information from the flow propagation module
        inpainting_memory_0 = torch.Tensor().to(device)
        inpainting_memory_1 = torch.Tensor().to(device)
        flow_feat = []
        ...
        for f in range(0, video_length):
            ...
            with torch.no_grad():
                #We take last 2 frames before the current one for the flow propagation part
                selected_imgs = imgs[:1, neighbor_ids[-2:]+[f], :, :, :]
                selected_masks = masks[:1, neighbor_ids[-2:]+[f], :, :, :]
                masked_imgs = selected_imgs*(1-selected_masks)

                #We do the same as for other models (using past frames memories and saving inpainting of the new frame)
                #But we have 2 memories because the focal transformers have 2 different scales
                #We also keep some flow information (for the flow propagation part)
                pred_img, _, attn_0, attn_1, flow_feat = model(masked_imgs,inpainting_memory_0[neighbor_ids+ref_ids],inpainting_memory_1[neighbor_ids+ref_ids], flow_feat)

                #Memories update with the new frame
                inpainting_memory_0 = torch.cat((inpainting_memory_0,attn_0.unsqueeze(0)))
                inpainting_memory_1 = torch.cat((inpainting_memory_1,attn_1.unsqueeze(0)))
        ...
        #This may be useful to lighten the GPU memory
        del inpainting_memory_0
        del inpainting_memory_1
        del flow_feat
        torch.cuda.empty_cache()

        ## Saving inpaintings
        if args.save_results:
            folder = os.path.join(args.save_folder,'E2FGVI_OM',video_name)
