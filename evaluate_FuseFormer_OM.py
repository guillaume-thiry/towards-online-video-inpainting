...
def main_worker():
    ...
    for video_no in range(video_num):
        ...
        #Here we store the previous inpaintings (trought the several layers of the transformers)
        ...
        for f in range(0, video_length):
            ...
            with torch.no_grad():
                current_frame = imgs[:1, [f], :, :, :]
                current_mask = masks[:1, [f], :, :, :]
                masked_img = current_frame*(1-current_mask)

                #Here we inpaint the last frame using inpainting memories of previous frames, and we retrieve the inpainting info of this new frame to add to the memory
                pred_img, attn = model(masked_img, inpainting_memory[neighbor_ids+ref_ids])
        ...
        if args.save_results:
            folder = os.path.join(args.save_folder,'FuseFormer_OM',video_name)
