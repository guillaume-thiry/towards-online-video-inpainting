...
from model.modules.feat_prop_M import BidirectionalPropagation, SecondOrderDeformableAlignment
from model.modules.tfocal_transformer_M import TemporalFocalTransformerBlock, SoftSplit, SoftComp
...
class InpaintGenerator(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        for i in range(depths): #Adding ids to the transformers, to identify then when loading inpainting memory
            blocks.append(TemporalFocalTransformerBlock(id=i,
    ...
    def forward_flow(self, masked_local_frames):
        ...
        if l_t>1:
            ...
            pred_flows_forward=None

        return pred_flows_forward

    def forward(self, masked_frames, former_attn_0, former_attn_1, feat_prop): #Adding memories of the previous frames as input (2 transformer memories + flow memory)
        attn_save_0 = torch.Tensor().to(masked_frames.device) #Adding these variables to save the inpainting of the new frame (to then save it)
        attn_save_1 = torch.Tensor().to(masked_frames.device)

        b, t, ori_c, ori_h, ori_w = masked_frames.size()

        # normalization before feeding into the flow completion module
        masked_local_frames = (masked_frames + 1) / 2
        pred_flows = self.forward_flow(masked_local_frames)
        ...
        enc_feat = enc_feat[:,-1:,:,:,:]
        enc_feat, feat_prop = self.feat_prop_module(enc_feat, pred_flows, feat_prop) #This has been simplified as we do not use backward flow here (we inpaint the last frame available)
        ...
        trans_feat, attn_save_0, attn_save_1, _, _ = self.transformer(trans_feat, attn_save_0, attn_save_1, former_attn_0, former_attn_1) #The memories are used here in the transformers and we retrieve the new inpainting info
        trans_feat = self.sc(trans_feat, 1)
        trans_feat = trans_feat.view(b, 1, -1, h, w)
        enc_feat = enc_feat + trans_feat


        # decode frames from features
        output = self.decoder(enc_feat.view(b, c, h, w))
        output = torch.tanh(output)
        return output, pred_flows, attn_save_0, attn_save_1, feat_prop #Same format as the input because it passes that to the next transformer in the sequence
