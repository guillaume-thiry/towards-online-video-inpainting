...
class InpaintGenerator(BaseNetwork):
    def __init__(self, init_weights=True):
        ...
        for k in range(stack_num): #Adding ids to the transformers, to identify then when loading inpainting memory
            blocks.append(TransformerBlock(id = k, hidden=hidden, num_head=num_head, dropout=dropout, n_vecs=n_vecs,
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, x, k):
        ...
        key = key.view(b, n, self.head, c_h).permute(0, 2, 1, 3) #Trimming the queries because we only want to inpaint the last frame
        query = self.query_embedding(x[:,:k,:])
        query = query.view(b, k, self.head, c_h).permute(0, 2, 1, 3)
        value = self.value_embedding(x)
        value = value.view(b, n, self.head, c_h).permute(0, 2, 1, 3)
        att, _ = self.attention(query, key, value)
        att = att.permute(0, 2, 1, 3).contiguous().view(b, -1, c)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, id, hidden=128, num_head=4, dropout=0.1, n_vecs=None, t2t_params=None):
        super().__init__()
    ...
    def forward(self, input, save_tensor, formers): #We add here the saving tensor (for the new inpainted frame) and the memory of the former inpaints
        ...
        if formers.numel(): #check if empty tensor or not
            load = formers[:,self.id].permute(1,0,2,3).contiguous().view(1,-1,self.hidden)
            x = torch.cat((x,load), 1)

        attn = self.dropout(self.attention(x,k))
        x = input + attn
        y = self.norm2(x)
        x = x + self.ffn(y)
