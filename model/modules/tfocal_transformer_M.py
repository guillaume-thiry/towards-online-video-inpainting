...
"""
## We modify this file for Online-Memory inpainting to deal with transformers memories (saving/loading) and inpaints only the last frame
import math
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x_all, mask_all=None):
        ...
        # Modification here to cut the queries vector to only the last frame
        q_windows = q_windows[:,:,:self.window_size[0] * self.window_size[1],:]
        attn = (q_windows @ k_all.transpose(-2, -1))  # B*nW, nHead, T*window_size*window_size, T*focal_window_size*focal_window_size
        # T * 45
        window_area = 1 * self.window_size[0] * self.window_size[1]
        # T * 165
...
class TemporalFocalTransformerBlock(nn.Module):
    ...
    def __init__(self, id, dim, num_heads, window_size=(5, 9), mlp_ratio=4., #Add id to identify the Transformers
    ...
    def forward(self, x, save_tensor_0, save_tensor_1, formers_0, formers_1): #Parameters here to save/load information from memory
        B, T, H, W, C = x.shape
        ...
        #Saving inpaintings in memory before the next transformer (current frame)
        save_tensor_0 = torch.cat((save_tensor_0,x_windows_all[0].unsqueeze(0)))
        save_tensor_1 = torch.cat((save_tensor_1,x_windows_all[1].unsqueeze(0)))

        #Loading inpaintings from memory before next transformer (previous frames)
        if formers_0.numel(): #check if empty tensor or not
            load = formers_0[:,self.id].permute(1,0,2,3,4,5).contiguous().view(1,-1,H,W,C)
            x_windows_all[0] = torch.cat((x_windows_all[0],load), 1)

        if formers_1.numel(): #check if empty tensor or not
            load = formers_1[:,self.id].permute(1,2,3,0,4,5).contiguous().view(1,nWh,nWw,-1,C)
            x_windows_all[1] = torch.cat((x_windows_all[1],load), 3)

        #Then procede as usual, with the only difference being the cut in the query vector (in attention calculation)
        ...
        return x, save_tensor_0, save_tensor_1, formers_0, formers_1
